(function () {
  ...
  const handleModalEvent = (event) => {
    ...
    if (target.closest('.about__img-button')) {
      event.preventDefault();
      body.classList.add('body--opened-modal');
    }
    else if (target.closest('.modal__cancel') || target.closest('.button') || !target.closest('.modal__window')) {
      event.preventDefault();
      body.classList.remove('body--opened-modal');
    }
  };

  document.addEventListener('click', handleModalEvent);
